...
interface PerformanceData {
    ...
    spAccuracy: {
        ...
        buckets: Array<{
            ...
            avgExpectedPerTask: number;
            avgActualPerTask: number;
            accuracy: number | null;
        }>;
        coaching: string[];
    };
...
function SpAccuracySection({ spAccuracy }: { spAccuracy: PerformanceData['spAccuracy'] }) {
    const { expectedHoursPerSP, teamCompletedPoints, teamWorklogHours, teamAvgHoursPerSP, teamAccuracy, members, buckets, coaching } = spAccuracy;

    // Corrected: 100% = perfect, <100% = under-logged, >100% = under-estimated
    function getColor(accuracy: number | null): string {
        if (accuracy === null) return 'text-muted-foreground';
        if (accuracy >= 90 && accuracy <= 110) return 'text-green-400';
        if (accuracy >= 70 && accuracy <= 130) return 'text-amber-400';
        return 'text-red-400';
    }
    ...
    function getBadge(accuracy: number | null): { label: string; className: string } {
        ...
        if (accuracy >= 90 && accuracy <= 110) return { label: 'Well Calibrated', className: 'bg-green-500/20 text-green-400' };
        if (accuracy < 90) return { label: 'Under-logged', className: 'bg-red-500/20 text-red-400' };
        return { label: 'Under-estimated', className: 'bg-amber-500/20 text-amber-400' };
    }
    ...
    return (
        <div className="space-y-5">
                ...
                <div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-border text-muted-foreground text-xs uppercase tracking-wider">
                                    <th className="text-left py-3 px-3">SP Value</th>
                                    <th className="text-right py-3 px-3">Tasks</th>
                                    <th className="text-right py-3 px-3">Expected (avg)</th>
                                    <th className="text-right py-3 px-3">Actual (avg)</th>
                                    <th className="text-right py-3 px-3">Accuracy</th>
                                </tr>
                            </thead>
                            <tbody>
                                {buckets.map((b, i) => (
                                    <tr key={i} className="border-b border-border/50 hover:bg-muted/20">
                                        <td className="py-3 px-3 text-foreground font-bold">{b.storyPoints} SP</td>
                                        <td className="py-3 px-3 text-right text-muted-foreground">{b.issueCount}</td>
                                        <td className="py-3 px-3 text-right text-muted-foreground">{b.avgExpectedPerTask}h</td>
                                        <td className="py-3 px-3 text-right text-foreground">{b.avgActualPerTask}h</td>
                                        <td className={`py-3 px-3 text-right font-bold ${getColor(b.accuracy)}`}>
                            ...
                            <tbody>
                                {members.map((m, i) => (
                                    <tr key={i} className="border-b border-border/50 hover:bg-muted/20">
                                        ...
                                        <td className="py-3 px-3 text-right text-muted-foreground">{m.completedPoints > 0 ? `${Math.round(m.completedPoints * expectedHoursPerSP * 10) / 10}h` : '—'}</td>
                                        <td className="py-3 px-3 text-right text-muted-foreground">{m.worklogHours > 0 ? `${m.worklogHours}h` : '—'}</td>
            ...
            <p className="text-[10px] text-muted-foreground">
                Baseline: 1 SP = 1 Manday = {expectedHoursPerSP}h. Accuracy = actual logged hours ÷ expected hours × 100%. 100% = perfect estimation. Aggregated across all sprints in the selected period.
            </p>
        </div>
    );
}
