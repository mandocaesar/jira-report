...
async function generateAndSendReportForBoard(
    boardId: number,
    emailGroup: string,
): Promise<{ boardId: number; status: string; sprintName?: string; sentTo?: string; reason?: string }> {
...
export async function POST(request: Request) {
    try {
        ...
        if (!emailGroup && isDatabaseAvailable() && prisma) {
        ...
        if (!emailGroup) {
            return NextResponse.json(
                { success: false, error: 'No email group configured for this board. Please configure it in Team Settings first.' },
                { status: 400 }
            );
        }
