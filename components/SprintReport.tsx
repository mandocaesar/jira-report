        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="text-center p-5 bg-gradient-to-br from-indigo-500/10 to-violet-500/10 rounded-xl border border-indigo-500/20">
                    ...
                    <div className="text-[10px] text-gray-500 mt-1">(sub-tasks)</div>
                </div>
                ...
                <div className="text-center p-5 bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-xl border border-purple-500/20">
                    <div className={`text-3xl font-bold mb-1 ${getCompletionColor(completionPercent)}`}>
            ...
            {statusGroups.length > 0 && (
                <div className="bg-gray-800/30 rounded-xl p-6 border border-gray-700/30">
                    <h3 className="text-sm font-semibold text-gray-400 mb-4">Status Breakdown</h3>
                    <div className="space-y-3">
                        {statusGroups.map((group) => {
                            const colors = getStatusColors(group.statusCategory);
                            const percentage = totalPoints > 0 ? (group.points / totalPoints) * 100 : 0;
                            return (
                                <div key={group.statusCategory} className="flex items-center gap-4">
                                    <div className={`w-28 text-sm font-medium ${colors.text} flex-shrink-0`}>
                                        {group.statusCategory}
                                    </div>
                                    <div className="flex-1">
                                        <div className="h-6 bg-gray-700/30 rounded-lg overflow-hidden relative">
                                            <div
                                    ...
                                    <div className="w-14 text-right text-sm font-bold text-gray-300">
            ...
            {memberBreakdowns.length > 0 && (
                <div className="bg-gray-800/30 rounded-xl border border-gray-700/30 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-700/30">
                        <h3 className="text-sm font-semibold text-gray-400">Per Team Member</h3>
                    </div>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                {member.user.avatarUrl ? (
                                                    ...
                                                    <div className="w-8 h-8 rounded-full ring-2 ring-gray-700 bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs">
                                                        {member.user.displayName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                                                    </div>
                                                )}
                                                <div>
                                                    <div className="text-sm font-medium text-white">{member.user.displayName}</div>
                                                    <div className="flex items-center gap-1.5">
                                                        <span className={`text-[10px] px-1.5 py-0.5 rounded ${member.role === 'qa'
                                                            ? 'bg-pink-500/20 text-pink-300 border border-pink-500/30'
                                                            : 'bg-blue-500/20 text-blue-300 border border-blue-500/30'
                                                            }`}>
                                                            {member.role.toUpperCase()}
                                                        </span>
                                                        <span className="text-[10px] text-gray-500">{member.title}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <span className="text-sm font-bold text-white">{member.totalPoints}</span>
                                        </td>
                                        <td className="px-4 py-4 text-center">
            ...
            {/* Empty state */}
            {totalPoints === 0 && (
                <div className="text-center py-12 bg-gray-800/20 rounded-xl border border-gray-700/20">
                    <div className="w-16 h-16 bg-gray-700/30 rounded-full flex items-center justify-center mx-auto mb-4">
                        <svg className="w-8 h-8 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                        </svg>
                    </div>
                    <p className="text-gray-400">No sub-tasks found in this sprint</p>
                    <p className="text-sm text-gray-500 mt-1">Sprint completion is calculated from sub-tasks only</p>
                </div>
            )}
        </div>
    );
